function [n,center,total_residual] = fitNormal(data, show_graph, room)
...
% :type n: 3x1 vector
	...
	center = mean(data);
	off_center_old = [data(:,1)-center(1) data(:,2)-center(2) data(:,3)-center(3)];
	norms = vecnorm(off_center_old');
    off_center = (off_center_old'./norms)';
    for i = 1:3
		if can_solve(i) == 0
			residual_sum(i) = NaN;
			continue
	...
	n = n(:,best_fit(1));
    total_residual = residual_sum(best_fit)/(size(data,1));
	...
	L=plot3(data(:,1),data(:,2),data(:,3),'ro','Markerfacecolor',room); % Plot the original data points
	hold on;
	...
	L=plot3(norm_data(:,1),norm_data(:,2),norm_data(:,3),'Color',room,'LineWidth',3);
